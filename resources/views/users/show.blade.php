    ...
    <div class="row justify-content-center">
        <form id="userForm" action="{{route('usuarios.update', $usuario)}}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="nombre" name="name"  value="{{ $usuario->name }}" {{ $edit ? 'required' : 'disabled' }}>
            </div>
            <div class="form-group">
                <label for="email">Email:</label>
                <div class="input-group">
                    <span class="input-group-text" id="inputGroupPrepend">@</span>
                    <input type="email" class="form-control" id="email" aria-describedby="inputGroupPrepend" name="email" value="{{ $usuario->email }}" {{ $edit ? 'required' : 'disabled' }}>
                </div>
            </div>
            <div class="form-group">
                ...
                <select name="roles[]">
                    ...
                    <option value="{{ $rol->name }}" {{ ($usuario->hasRole($rol->name)) ? 'selected' : '' }} {{ $edit ? 'required' : 'disabled' }}>{{ $rol->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group {{ $edit ? 'd-block' : 'd-none' }}" >
                ...
                <div class="input-group" >
                    <input type="password" class="form-control" id="password" name="password" required value="{{ $usuario->password }}" {{ $edit ? 'required' : 'disabled' }}>
            ...
            <div class="form-group {{ $edit ? 'd-block' : 'd-none' }}">
                ...
                <div class="input-group">
                    <input type="password" class="form-control" id="confirmPassword" name="confirmPassword" value="{{ $usuario->password }}" {{ $edit ? 'required' : 'disabled' }}>
